...
if (defined('ROOT_PATH')) {
    ...
    echo '<h2>'.install_text('Проверка прав доступа к файлам', 'Verify file and folder permissions').'</h2>';
    echo '<p>'.install_text('Убедитесь, что перечисленные каталоги созданы и доступны для записи.', 'Ensure the listed directories exist and are writable.').'</p>';
    echo '<ul>';
    $error = false;
    $folders = [
        ROOT_PATH.'datas/',
    ...
    foreach ($folders as $folder) {
        ...
        $label = $isWritable
            ...
            : install_text('Каталог <strong>'.str_replace(ROOT_PATH, '', $folder).'</strong> недоступен для записи. Создайте его и установите права 755.', 'Folder <strong>'.str_replace(ROOT_PATH, '', $folder).'</strong> is not writable. Create it and apply chmod 755.');
    ...
    foreach ($files as $file) {
        ...
        $label = $isWritable
            ...
            : install_text('Файл <strong>'.str_replace(ROOT_PATH, '', $file).'</strong> не удаётся создать. Создайте его вручную и назначьте права 755.', 'File <strong>'.str_replace(ROOT_PATH, '', $file).'</strong> cannot be created. Create it manually and set chmod 755.');
        echo '<li class="'.$statusClass.'">'.$icon.'<span>'.$label.'</span></li>';
        $error = !$isWritable || $error;
    }
    echo '</ul>';
    echo '<div class="button-bar">';
    echo '<a class="button secondary" href="index.php?step=1&amp;lang='.$lang.'">'.install_text('Проверить снова', 'Check again').'</a>';
    if (!$error) {
        echo '<a class="button primary" href="index.php?step=2&amp;lang='.$lang.'">'.install_text('Продолжить', 'Continue').'</a>';
    }
    echo '</div>';
}
